<template>
  <div class="container" >
    <h1 class="subheading text-center">Login Logout page</h1>
     <v-snackbar
     class="mb-5"
      v-model="snackbar"
      :timeout="2000"
...
const Mappers = Vue.extend({
  computed: {
    ...UserStore.mapGetters(["responseError", "userInfo", "loggedIn"])
  },
  methods: {
...
export default class LoginLogout extends Mappers {
  ...
  @Emit()
  async submit(){
    // console.log(this.responseError, this.userInfo, this.loggedIn)
     this.errorText = this.responseError
    if(this.errorText==="Login or password is incorrect"){   
      this.snackbar = true;
      }
